...
def creation_table(nom_table):
    ...
    global curseur
    global connexion
    requetes = {'Type' : """CREATE TABLE Type ('ID_Type' SMALLINT PRIMARY KEY, 'Nom' VARCHAR(20), 'Casse' TEXT);"""
                , 'Matériau' : """CREATE TABLE Matériau ('ID_Matériau' TEXT PRIMARY KEY, 'Nom' TEXT, 'Image' TEXT);"""
                , 'Outil' : """CREATE TABLE Outil ('ID' TEXT PRIMARY KEY, 'ID_Type' INTEGER REFERENCES Type('ID_Type'), 'ID_Matériau' TEXT REFERENCES Matériau('ID_Matériau'), 'Nom' TEXT, 'Durabilité' INTEGER, 'Dégâts' INTEGER, 'Image' TEXT);"""}
...
def filtre_distinct(categories1, nom_table, categorie2, DESC_ou_ASC):
    '''
